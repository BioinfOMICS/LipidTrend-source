...
for(split in c(TRUE, FALSE)){
  ...
  if(split){
    ...
    data.table::fwrite(reg.tab$odd_result, file.path(outPATH, "totalChain_1D_oddChain_regionalTestFC_result.csv"))
    ...
    # Region-level paired t-test and fold-change (new in LipidTrend v1.3.1)
    reg.tab <- LipidTrend::regionalTestFC(res, p_cutoff=0.05)
    data.table::fwrite(reg.tab, file.path(outPATH, "totalChain_1D_regionalTestFC_result.csv"))
