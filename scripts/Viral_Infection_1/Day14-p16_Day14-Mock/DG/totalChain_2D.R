...
for(split in c(TRUE, FALSE)){
  ...
  if(split){
    ...
    # Save tables
    data.table::fwrite(res.tab, file.path(outPATH, "totalChain_2D_smoothing_result.csv"))

    # Region-level paired t-test and fold-change (new in LipidTrend v1.3.1)
    reg.tab <- LipidTrend::regionalTestFC(res, p_cutoff=0.05)
    data.table::fwrite(reg.tab, file.path(outPATH, "totalChain_2D_regionalTestFC_result.csv"))
 
    # Save plots
    ggplot2::ggsave(file.path(outPATH, "totalChain_2D_plot.pdf"), plot, width = 8, height = 4.5)
  }
